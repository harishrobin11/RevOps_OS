...
def render_lead_detail_page(lead_id: int):
    ...
    if not lead:
        ...
        if st.button("🔙 Back to Lead Registry"):
    ...
    st.markdown('<div class="section-card"><h3>📌 Executive Account Intelligence</h3>', unsafe_allow_html=True)
    ...
    with card_col1:
        ...
        st.markdown(f"**ICP Score:** `{lead['icp_score']:.0f}/100`")
    ...
    with card_col2:
        ...
        st.markdown(f"**Persona Type:** `Decision Maker`")

    with card_col3:
        qual = lead.get("qualification") or {}
        bant_score = qual.get("bant_score", 0.0)

        st.markdown("##### 📊 Qualification & Deal Metrics")
        st.markdown(f"**BANT Score:** `{bant_score:.0f}/100`")
        st.markdown(f"**Priority Tier:** `{lead['priority']}`")
        st.markdown(f"**Pipeline Stage:** `{lead['pipeline_stage']}`")
        st.markdown(f"**Deal Health:** `{lead['deal_health']}`")
        st.markdown(f"**Deal Value:** `{format_currency(lead['deal_value'])}`")
        st.markdown(f"**Win Probability:** `{lead['probability']*100:.0f}%`")

    st.markdown('</div>', unsafe_allow_html=True)

    # CONTEXT & PAIN POINTS CARD
    st.markdown('<div class="section-card"><h3>📝 Operational Context & Pain Points</h3>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-card"><h3>🎯 BANT Qualification Evaluator</h3>', unsafe_allow_html=True)
    ...
    with st.form("bant_eval_form"):
        st.markdown("##### Evaluate BANT Parameters")
        ...
        save_qual = st.form_submit_button("💾 Update BANT Qualification & Recalculate Priority", use_container_width=True)
    ...
    st.markdown('<div class="section-card"><h3>⚡ Log Activity & Add Notes</h3>', unsafe_allow_html=True)
    ...
    with act_tab:
        with st.form("log_activity_form", clear_on_submit=True):
            ...
            submit_act = st.form_submit_button("Log Activity", use_container_width=True)
            if submit_act:
    ...
    st.markdown('<div class="section-card"><h3>📜 Activity Timeline & Audit History</h3>', unsafe_allow_html=True)
    ...
    if notes_list:
        st.markdown("##### 📝 Internal Notes Log")
        for n in notes_list:
