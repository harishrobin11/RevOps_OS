...
def test_csv_validation_and_parsing():
    ...
    sample_csv = f"""Company,Contact,Title,Email,Phone,Industry,Location,Employees,Lead Source,Deal Value,Notes
Unique Test Corp {uid},Aman Gupta,Head of Operations,aman_{uid}@uniquetest101.com,+91 99999 11111,SaaS,Outer Ring Road,120,Outreach,1800000,Testing notes
,No Company Contact,VP Operations,nocompany@test.com,+91 99999 22222,FinTech,Koramangala,80,Inbound,1500000,No company
Invalid Email Inc {uid},Suresh,Manager,invalid_email_at_test.com,+91 99999 33333,HealthTech,HSR Layout,50,CSV Import,1000000,Bad email
"""
    valid_rows, error_log = validate_and_parse_csv(sample_csv)

    assert len(valid_rows) == 1
    assert valid_rows[0]["company_name"] == f"Unique Test Corp {uid}"
    assert valid_rows[0]["email"] == f"aman_{uid}@uniquetest101.com"
...
def test_import_leads_execution():
    uid = uuid.uuid4().hex[:6]
    comp_alpha = f"New Import Alpha {uid}"
    comp_beta = f"New Import Beta {uid}"
    test_csv = f"""Company,Contact,Title,Email,Phone,Industry,Location,Employees,Lead Source,Deal Value,Notes
{comp_alpha},Rohan Verma,COO,rohan_{uid}@alphanew.com,+91 98888 77777,AI & Automation,Electronic City,150,Inbound,2500000,Scaleup operational bottleneck
{comp_beta},Simran Kaur,Head of Delivery,simran_{uid}@betanew.com,+91 98888 66666,SaaS,Whitefield,90,Outreach,1800000,Need process visibility
"""
    ...
    with get_db() as session:
        lead_alpha = session.query(Lead).filter(Lead.company_name == comp_alpha).first()
